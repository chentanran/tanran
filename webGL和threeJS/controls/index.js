...
function removeAndAdd(item, value, camera, mesh, scene, controls) {
  ...
  for (const key in controls) {
    if (roundValue[key]) {
      console.log(controls[key], ~~controls[key])
  ...
  if (isFont(item)) {
    mesh.pointer = createMaterial(new THREE[item.type]('123', Object.assign(textOptions, controls)))
  } else {
...
const itemType = {
  ...
  PlaneGeometry: ['width', 'height', 'widthSegments', 'heightSegments'], // 二维平面几何体
